...
$sorgu = $db->prepare('SELECT * FROM m_b');
$sorgu->execute();
$musterilist = $sorgu->fetchAll(PDO::FETCH_OBJ);

$query = $db->query('SELECT * FROM musteri_cd');
$query1 = $db->query('SELECT * FROM dt_gore_musteri_s');
   ...
   <head>
      ...
      <script type="text/javascript" src="https://www.gstatic.com/charts/loader.js"></script>
         <script type="text/javascript">
            ...
            var options = {'title':'Müşterilerin Cinsiyetlerine Göre Dağılımı',
                           'width':500,
                           'height':400};
            var chart = new google.visualization.PieChart(document.getElementById('mygd_chart'));
         ...
         <script type="text/javascript">
            ...
            data.addRows([
                  ...
                  foreach($query1 as $row1) {
                     echo "['".$row1["yil"]."',".$row1["musteriSayisi"]."],";
                  }
                  ?>
            ]);
            var options = {'title':'Doğum Yıllarına Göre Müşteri Dağılımı',
                           'width':500,
                           'height':400};
            var chart = new google.visualization.PieChart(document.getElementById('mdygs_chart'));
            chart.draw(data, options);
            }
         </script>
   </head>
                                 ...
                                 <div class="table_section padding_infor_info">
                                    <div class="table-responsive-sm">
                                       <table class="table table-hover">
                                          ...
                                          <?php foreach ($musterilist as $mlist){?>
                                          <tbody>
                                             <tr>
                                                <td><?= $mlist->musteri_id ?></td>
                                                <td><?= $mlist->musteri_ad ?></td>
                                                <td><?= $mlist->musteri_soyad ?></td>
                                                <td><?= $mlist->cinsiyet_ad ?></td>
                                                <td><?= $mlist->musteri_dog_tar ?></td>
                                                <td><?= $mlist->musteri_tel ?></td>
                                                <td><a href="sil.php?musterilerid=<?= $mlist->musteri_id ?>" class="red_color font-weight-bold">Sil</a></td>
                                             </tr>
                                          </tbody>
                                          <?php } ?>
                                       </table>
